...
BamWriteCompress::BamWriteCompress(int BufferSize, int threadNumber) {
    ...
    for (int i = compress_bg; i <= compress_ed; i++) {
        ...
        compress_data[i]->uncompressed_data = (uint8_t*)aligned_alloc_custom(64, BGZF_BLOCK_SIZE);
        compress_data[i]->compressed_data = (uint8_t*)aligned_alloc_custom(64, BGZF_MAX_BLOCK_SIZE);
    }
...
bam_write_block *BamWriteCompress::getUnCompressData() {
    ...
    while (1) {
        ...
        if (done) {
#ifdef DEBUG
            printf("DEBUG getUnCompressData No more data, blockInputPos=%d blockInputNum=%d\n",
                   (int)blockInputPos.load(), blockInputNum);
#endif
            break;
        }
...
#ifdef DEBUG
            printf("DEBUG getUnCompressData Got block: block_num=%d block_offset=%d\n",
                   result ? result->block_num : -1, result ? result->block_offset : -1);
#endif
            break;
        }
    }
...
void BamWriteCompress::inputCompressData(bam_write_block *data) {
    int current_blockNum = blockNum.load(std::memory_order_acq_rel);
#ifdef DEBUG
    printf("DEBUG inputCompressData Input block: block_num=%d current_blockNum=%d block_length=%d\n",
           data->block_num, current_blockNum, data->block_length);
#endif
    while (data->block_num != current_blockNum) {
        wait_num += 1;
        std::this_thread::sleep_for(std::chrono::nanoseconds(data->block_num - current_blockNum) / 8);
        current_blockNum = blockNum.load(std::memory_order_acq_rel);
    }
...
#ifdef DEBUG
    printf("DEBUG inputCompressData Added to consumer queue: block_num=%d consumer_ed=%d\n",
           data->block_num, consumer_ed);
#endif
}
...
bam_write_block *BamWriteCompress::getCompressData() {
    while ((consumer_ed + 1) % consumer_size == consumer_bg) {
        std::this_thread::sleep_for(std::chrono::nanoseconds(1));
        if (compressThread == 0 && (consumer_ed + 1) % consumer_size == consumer_bg) {
#ifdef DEBUG
            printf("DEBUG getCompressData No more data, queue empty (compressThread=0)\n");
#endif
            return nullptr;
        }
    }
    ...
    bam_write_block *result = consumer_data[num];
#ifdef DEBUG
    printf("DEBUG getCompressData Got compressed block: block_num=%d block_length=%d\n",
           result ? result->block_num : -1, result ? result->block_length : -1);
#endif
    return result;
}
