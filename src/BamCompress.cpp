...
void BamCompress::resize(int BufferSize, int threadNumber) {
    ...
    for (int i = compress_bg; i <= compress_ed; i++) {
        compress_data[i] = new bam_block;
        // Allocate data buffer with 64-byte alignment for Sunway slave cores
        compress_data[i]->data = aligned_alloc_custom(64, BGZF_MAX_BLOCK_SIZE);
        if (!compress_data[i]->data) {
            // Allocation failed - handle error appropriately
            delete compress_data[i];
            compress_data[i] = nullptr;
        }
    }
